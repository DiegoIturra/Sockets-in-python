...
class ServerUDP:
    ...
    def __send_file(self,filename,address):
        ...
        progress = tqdm.tqdm(range(filesize) , f"Sending {filename} to {address}" , unit="B",unit_scale=True,unit_divisor=1024)
    ...
    def __handle_request(self,filename,address):
        with self.lock:
            filesize = self.__get_file_size(filename)
            ...
            self.__decrypt_file(filename) #Desencripta archivo original
        ...
        while True:
            ...
            if data:
                ...
                if self.__exist_file(filename):
                    thread = Thread(target=self.__handle_request , args=(filename,address))
                    thread.daemon = True
                    thread.start()
                else:
